...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        const hasSeeded = await AsyncStorage.getItem('tailorp_seeded');
        ...
        if (!hasSeeded && parsedCustomers.length === 0) {
          const now = Date.now();
          ...
          await AsyncStorage.setItem('tailorp_seeded', 'true');
          await AsyncStorage.setItem('tailorp_customers', JSON.stringify(newCustomers));
          await AsyncStorage.setItem('tailorp_orders', JSON.stringify(newOrders));
          await AsyncStorage.setItem('tailorp_appointments', JSON.stringify(newAppointments));
        }
      } catch (e) {
  ...
  const seedSampleData = useCallback(() => {
    ...
    AsyncStorage.setItem('tailorp_seeded', 'true').catch(e => console.error(e));
  }, [customers, orders]);
